...
* target 属性：指定链接页面打开方式，_ self 为默认值-当前页面，_ blank 为在新窗口中打开方式
...
* 一般情况下，`<input>`类表单元素才能获取，因此该选择器也主要针对于表单元素
  ...
  ```html
  input:focus {  /* 获得光标的输入框的背景颜色变为粉红色 */
...
* **外边距合并：**
  ...
  2. **嵌套块元素垂直外边距塌陷：** 两个拥有父子关系的块元素中，在父元素拥有外边距且子元素也拥有外边距时，比较两个外边距的大小，最终取外边距较大值为父元素真正实现的外边距，子元素依旧紧贴父元素
     * 举例：父元素 margin-top: 50px; 子元素 margin-top: 100px;最终实现效果是父元素外边距为100px，子元素紧贴父元素
     * 解决方案：
       * 为父元素定义边框
       * 为父元素定义内边距
...
* 约束浮动元素位置，网页布局策略：先用标准流父元素排列上下位置，再用内容子元素采取浮动排列左右位置，复合网页布局第一准则
...
* 相对定位特点：
  ...
  2.  原来在标准流中的位置继续保留，后面盒子不占据它在标准流中的位置**（不脱标，继续保留原来位置）**
* 最典型应用：作为绝对定位的父元素使用
...
* visibility 与 display 区别：
  * 隐藏元素后占有原来位置，使用 visibility：hidden
  * **隐藏元素后不占有原来位置，使用 display：none（用处更多，重要）**
